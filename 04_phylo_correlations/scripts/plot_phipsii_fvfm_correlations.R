...
for(i in 1:length(lifestages)) {
  ...
  filename <- paste0("../output/phipsii_fvfm_",lifestages[i],"/multivariate_BM.log")
  print(filename)
...
stats %>%
  mutate("Life stage" = case_when(
    file_name == "phipsii_fvfm_seedling/output/multivariate_BM.log" ~ "1", 
    file_name == "phipsii_fvfm_young/output/multivariate_BM.log" ~ "2", 
    file_name == "phipsii_fvfm_old/output/multivariate_BM.log" ~ "3", 
    file_name == "phipsii_fvfm_haustorium/output/multivariate_BM.log" ~ "4", 
    file_name == "phipsii_fvfm_flower/output/multivariate_BM.log" ~ "5", 
    file_name == "phipsii_fvfm_seed/output/multivariate_BM.log" ~ "6"), .before = file_name) %>% select("Life stage", BF_correlated, BF_positive, BF_negative) -> stats
...
df %>%
  mutate("grp" = case_when(
    ...
    Life.stage == "Flower" ~ "5", 
    Life.stage == "Seed" ~"6")) %>% select(grp, Correlation) -> df
...
pdf("../output/plots/correlations_phipsii_fvfm_density_violin.pdf", width=6,height=5) 
density_viol
dev.off()
