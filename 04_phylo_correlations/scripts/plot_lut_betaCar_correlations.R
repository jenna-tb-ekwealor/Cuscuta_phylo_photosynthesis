...
for(i in 1:length(lifestages)) {
  ...
  filename <- paste0("../output/lut_bCar_",lifestages[i],"/multivariate_BM.log")
  print(filename)
...
filut_name <- as.vector(unlist(tissue_name))
...
stats <- data.frame(filut_name, BF_correlated, BF_positive, BF_negative)

stats %>%
  dplyr::mutate("Life stage" = case_when(
    filut_name == "../output/lut_bCar_seedling/multivariate_BM.log" ~ "1", 
    filut_name == "../output/lut_bCar_young/multivariate_BM.log" ~ "2", 
    filut_name == "../output/lut_bCar_old/multivariate_BM.log" ~ "3", 
    filut_name == "../output/lut_bCar_haustorium/multivariate_BM.log" ~ "4", 
    filut_name == "../output/lut_bCar_flower/multivariate_BM.log" ~ "5", 
    filut_name == "../output/lut_bCar_seed/multivariate_BM.log" ~ "6"), .before = filut_name) %>% dplyr::select("Life stage", BF_correlated, BF_positive, BF_negative) -> stats
...
pdf("../output/plots/correlations_lut_b.Car_density_violin.pdf", width=6,height=5) 
density_viol
dev.off()
