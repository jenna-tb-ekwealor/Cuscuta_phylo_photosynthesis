...
total_outliers_count <- length(outliers[["Cuscuta"]][["NPQ"]]) + length(outliers[["Cuscuta"]][["Fv.Fm"]]) + length(outliers[["Cuscuta"]][["φPSII"]]) +
  length(outliers[["Grammica"]][["NPQ"]]) + length(outliers[["Grammica"]][["Fv.Fm"]]) + length(outliers[["Grammica"]][["φPSII"]]) +
  length(outliers[["C_purpurata"]][["NPQ"]]) + length(outliers[["C_purpurata"]][["Fv.Fm"]]) + length(outliers[["C_purpurata"]][["φPSII"]]) +
  length(outliers[["Monogynella"]][["NPQ"]]) + length(outliers[["Monogynella"]][["Fv.Fm"]]) + length(outliers[["Monogynella"]][["φPSII"]]) +
  length(outliers[["Ipomoea_nil"]][["NPQ"]]) + length(outliers[["Ipomoea_nil"]][["Fv.Fm"]]) + length(outliers[["Ipomoea_nil"]][["φPSII"]]) 
...
total_measurement_count <- length((data_long %>% dplyr::filter(Metric == "Fv.Fm"|Metric == "φPSII"| Metric == "NPQ"))$Value)
...
plotted_mets <- c("Fv.Fm", "φPSII", "NPQ")
...
write.xlsx(excel_list[["NPQ"]], file="../output/stat_results/pairwise_allmetsxtissues.xlsx", sheetName="NPQ", append=TRUE, row.names=FALSE, col.names=FALSE)
