...
get_cbd_from_dunn <- function(name, dunn_object) {
  ...
  group_names <- unique(unlist(comps))  # preserve first-seen order

  # Initialize matrix with NA
  pmat <- matrix(NA, nrow = length(group_names), ncol = length(group_names),
  ...
  # Generate CLD
  cld <- QsRutils::make_letter_assignments(pmat)[["Letters"]]

  # Return named data frame, preserving row order and names
  cbd <- data.frame(Tissue.code = factor(names(cld), levels = names(cld)), Letter = unname(cld))
  return(cbd)
}
